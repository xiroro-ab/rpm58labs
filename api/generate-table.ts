...
PENGGABUNGAN (MERGE) — WAJIB: Jika ada dua baris atau lebih YANG BERURUTAN dengan Materi Pokok DAN TP yang sama persis, gabungkan sel Materi Pokok dan sel TP dengan atribut rowspan="N". N = jumlah baris yang sama, MAKSIMUM 5. Jika lebih dari 5, mulai grup baru lagi dari baris ke-6. Untuk sel yang tergabung, tulis teksnya SEKALI di baris pertama. Jika Materi Pokok atau TP-nya BERBEDA, baris dipisah seperti biasa. Jangan menambah properti page-break-inside: avoid pada tabel kisi-kisi (biarkan mengalir lintas halaman; tidak boleh menyisakan halaman kosong).
