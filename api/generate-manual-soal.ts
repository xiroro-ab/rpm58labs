...
`;
    ...
    const soalExample = `
<b>1.</b> [Pertanyaan pilihan ganda]
<table style="width: 100%; border-collapse: collapse; border: none; font-size: 10.5pt; margin: 4px 0 10px 0;">
  ...
  <tr>
    ...
    const prompt = `Anda adalah ahli penyusun lembar soal (penilaian sumatif).
...
Tata ulang soal tersebut menjadi Lembar Soal HTML yang rapi dengan PENOMORAN BERURUTAN 1, 2, 3, ... secara menerus.

FORMAT SOAL PILIHAN GANDA — salin pola kolom opsi 2x2 seperti contoh berikut:
${soalExample}
...
Catatan: Analisis teks soal manual untuk menentukan kunci jawaban yang paling tepat. Untuk soal uraian, kolom "Kunci Jawaban / Poin Kunci" diisi ringkasan poin-poin kunci jawaban (bukan jawaban panjang).
...
- JANGAN pakai page-break-inside: avoid pada kotak pertanyaan atau tabel (membuat banyak halaman kosong di PDF).
...
`;
    ...
    console.error('Error generating manual soal:', error);
    if (!res.headersSent) {
      res.status(500).json({ error: 'Gagal membuat lembar soal manual. Silakan coba lagi. Detail: ' + (error.message || 'Unknown error') });
    } else {
      res.end();
    }
  }
