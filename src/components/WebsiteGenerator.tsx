...
export default function WebsiteGenerator({ rpmHtml, topic, customApiKey, aiProvider = 'gemini', aiModel = '' }: WebsiteGeneratorProps) {
  ...
  const reviseWebsite = async () => {
    ...
    try {
      const res = await fetch('/api/revise-website', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ html: websiteHtml, instruction: chatInput, customApiKey, aiProvider, aiModel }),
      });
      const data = await res.json();
      if (data.html) {
        setWebsiteHtml(data.html);
